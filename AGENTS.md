...
**Comments** are optional, one to three short sentences on a single work (capped at 400 characters, cut at a sentence). The prompt asks for them on the two to four works Curio wants to point at, leading with the point, because the wall shows only the first few words: a blue tag at the foot of the picture (`ArtworkCard`) that opens to the full text inside the card's mat on hover (90ms intent) or focus, over the picture, so the masonry never moves. The detail view shows the comment under the title as "Curio on this work"; the thread's exhibit strip marks commented works with an accent square.
