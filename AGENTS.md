...
**Comments** are optional, one to three short sentences on a single work (capped at 400 characters, cut at a sentence). The prompt asks for them on the two to four works Curio wants to point at, leading with the point, because it types out from the start. At rest a commented work shows only a 3px accent line on its frame's top edge (`ArtworkCard`). On hover (120ms intent) or focus the comment opens outside the frame: beside it where the wall has room (right, else left, measured against the grid), else above it (below when the picture is near the top of the window). It's ink on paper at 14px, with a 1px ink border and the same accent top edge, and it writes itself out in about a second at most. The full text is laid out invisibly first, so the box never grows, and reduced motion shows it at once. It floats over the neighbouring cards (`pointer-events: none`, the figure raised while open), so the masonry never moves. The detail view shows the comment under the title as "Curio on this work"; the thread's exhibit strip marks commented works with an accent square. A past exhibit in the thread has a "Show on the wall" button (`WallButton`).
