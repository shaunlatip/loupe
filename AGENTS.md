...
**Comments** are optional, one or two short sentences on a single work (under 30 words in the prompt; capped at 220 characters, cut at a sentence). The prompt asks for them on the two to four works Curio wants to point at, leading with the point, because it types out from the start. At rest a commented work shows a small accent badge (a note glyph, 2px paper rim so it reads on dark pictures too) in the top right corner of its frame (`ArtworkCard`). On hover (120ms intent) or focus the comment opens outside the frame: beside it where the wall has room (right, else left, measured against the grid), else above it (below when the picture is near the top of the window). It's a `CommentCard` (ink on paper at 14px, 1px ink border, accent top edge, "Curio" in the accent) and it writes itself out in a quarter of a second at most. The full text is laid out invisibly first, so the box never grows, and reduced motion shows it at once. It floats over the neighbouring cards (`pointer-events: none`, the figure raised while open), so the masonry never moves. The detail view shows the same card under the title, labelled "Curio on this work"; the thread's exhibit strip marks commented works with an accent square. A past exhibit in the thread has a "Show on the wall" button (`WallButton`).
